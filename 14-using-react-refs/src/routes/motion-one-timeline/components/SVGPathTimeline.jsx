...
function SVGPathTimeline({ size = 60 }) {
  ...
  const handleTimelineAnimate = () => {
    ...
    // animate(circleElement, { strokeDashoffset: [1, 0], visibility: 'visible' });
    const sequence = [
      [circleElement, { strokeDashoffset: [1, 0], visibility: 'visible' }],
      [pathElement, { strokeDashoffset: [1, 0], visibility: 'visible' }],
    ];
    timeline(sequence);
  };
